...
async function getCategoryIds() {
  ...
  return _.sampleSize(catIds, categoriesNum);
}
...
async function getCategory(catId) {
  ...
  let randomClues = _.sampleSize(cat.clues, categoryClues).map(c => ({
    question: c.question,
    answer: c.answer,
    showing: null
  }));
...
async function fillTable() {
  ...
  $("#game tbody").empty();
  for (let clueIdx = 0; clueIdx < categoryClues; clueIdx++) {
    let $tr = $("<tr>");
    for (let catIdx = 0; catIdx < categoriesNum; catIdx++) {
      $tr.append(
        $("<td>")
